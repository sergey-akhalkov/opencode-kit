...
- After compaction or when a new session receives that matrix, verify every candidate against `Original User Goal` before substantial work. Matrix entries are candidates, not selected actions. End the reflection with `Next-Session Action: <exactly one Working Repository improvement or none>`; never select a list or one action per direction. When the live-attempt gate is `blocked` or `unknown`, `Next Strategy` and `Next-Session Action` SHALL name the same first gate-closing offline step; a matrix item cannot preempt it, and another high-cost live attempt is prohibited. Otherwise execute the one highest-ROI improvement only when it directly accelerates the original goal, then continue the accepted outcome. If the goal is missing, ambiguous, or conflicts with a candidate, call `session_delivery_context` once when available and use root user messages/requirement signals to resolve it; do not launch a reviewer solely for compaction. Keep kit candidates visible but never make a non-blocking kit improvement the next action while unrelated project work is incomplete unless the owner explicitly included kit work. Instruction/process candidates still require a dedicated workflow comparison before retention.
