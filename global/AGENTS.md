...
- Develop proof runners, capture/evaluator utilities, restoration tools, and related instrumentation in the working project's established tooling tree, using `tools/proofs/**` when no stronger project convention exists. Do not leave their only source copy in OS temp, target/build output, tool-output, or another disposable directory; those locations are for raw evidence and disposable runtime state only.
- Before creating proof code, inventory the project for an existing runner or library and reuse or extend it. Preserve recurring boundary knowledge as the smallest cohesive API, interface, or library that removes demonstrated duplication; keep scenario-specific orchestration thin and do not add speculative abstraction unrelated to observed proof work.
- Maintain a discoverable project-native proof inventory, normally `tools/proofs/README.md`, listing each runner/library, owned boundary, modes, exact invocation, inputs, authorization or external effects, immutable evidence, restoration/cleanup, and known limits. Update the inventory in the same change as proof tooling so later sessions can find and reuse it without reconstructing prior work.
- Treat proof tooling as maintained source: use stable explicit I/O, privacy-safe output, deterministic cleanup, and clear Product Candidate / Proof Runner / Evaluator / Environment roles. Keep useful runners after the proof unless the project already supplies an equivalent; delete only disposable state and superseded duplicates.

### Shift-left real-boundary testing

- For every behavior dependency chain, minimize **time-to-first-real-signal**: choose and execute the earliest safely reachable real boundary on this fidelity ladder: deterministic offline/preserved replay -> local integration/simulator -> shadow or independently effect-blocked read-only real dependency -> bounded live effects -> end-to-end user/operator workflow. Fast unit/mock/component checks remain useful but are not real-boundary proof. The autonomous target is the highest rung allowed by current authority; for an owner-controlled rung, prepare the gate immediately but do not execute it. Once a safe real rung is reachable and authorized, run it before adding more behavior that depends on its still-unverified assumptions; do not defer it to phase end or feature completeness.
- Every roadmap or plan SHALL state the fidelity ladder once, then every behavior-slice item SHALL include the labeled fields `Current Rung`, `Next Real Boundary`, `Blocker/Unblocker` (`none` only when already reachable), and `Observable Proof`, plus owner authorization, safeguards, restoration/cleanup, and expected immutable evidence. Omission makes the plan incomplete. If unknown or mismatched real behavior can invalidate downstream work, stop only that dependency chain and request the exact owner-controlled action at its first safe gate; continue independent work that does not rely on the uncertainty.
...
- Shift-left sequencing does not authorize external operations or weaken protected boundaries, the `Live-Attempt Gate`, independent physical-effect suppression, identity/liveness, required restoration/cleanup, cost controls, or remote/destructive/deploy/install/release rules. When the next rung is owner-controlled, prepare its safe runbook and evidence path immediately, record the gate, and keep the affected outcome `development` or incomplete until authorized proof exists.
...
- One evidence-only failure after an external, physical, costly, destructive, or long-running attempt immediately blocks another live attempt through the same proof path. Before unblocking it, run the candidate post-run/evaluator chain against the preserved bundle without live effects through its terminal verdict and every downstream stage reachable for the actual run mode, including non-side-effecting finalization checks; fixing or testing only the first failing line, helper, or parser is insufficient. Replay prior bundles from the same failure chain. Unlock only when that preserved-corpus replay is green or the exact missing raw observation is identified. If the observation can only be acquired live, the next attempt is bounded evidence capture, not proof, and must record that limit in advance.
...
- Every compaction summary SHALL state `Original User Goal` from root user messages and `Goal Status`, then include `Session Reflection`: outcome, the slowest observed loop or largest time sink, concrete evidence, likely cause with uncertainty, and what should not be repeated. It SHALL emit `Live-Attempt Gate: clear | blocked | unknown`; for `blocked` or `unknown`, also emit `Failure Chain`, `Preserved Raw Bundles`, `Offline Replay Coverage`, `Terminal Replay Result`, and `Unlock Condition`. Missing gate evidence is `unknown`, never inferred `clear`.
- Every summary SHALL also include one compact improvement matrix with rows `Quality`, `Cycle Speed`, and `Token Economy`, and columns `Working Repository` and `opencode-kit`. Each of the six cells records `evidence -> smallest cheap improvement -> expected benefit -> cost/risk`, or `none` when the session supplies no evidence. Admit a candidate only when it has observed evidence, a direct causal link to `Original User Goal`, a local/reversible low-cost action, and no scope expansion; do not manufacture generic best practices.
- As soon as a candidate passes that gate and an identifiable writable active OpenSpec change owns it, append **every** admitted candidate under `## Session-Derived Improvements` in that change's `tasks.md`; do not wait for compaction or select only the highest-ROI item. Each unchecked task SHALL state `Trigger/Evidence`, `Why`, `Prerequisites`, `Scope/Non-Goals`, `Implementation`, `Observable Proof`, and `Validation`, plus `Owner Blocker` only when applicable. It becomes accepted completion scope and remains unchecked until implemented and proven. If the candidate targets another repository, changes accepted outcome, or crosses a protected boundary, do not mutate silently: record the exact target/owner blocker and require an authorized implementation path or explicit owner scope change before normal completion.
- After compaction or when a new session receives that matrix, verify every candidate against `Original User Goal` and reconcile all still-admissible entries before substantial work. Compaction itself cannot call tools, so it SHALL emit every not-yet-persisted record under `Pending Improvement Tasks` using the same fields; the next session SHALL persist all of them, not silently discard the non-selected rows. If no active OpenSpec change exists, preserve them there until an owning change or explicit owner disposition exists. Instruction/process candidates still require a dedicated workflow comparison before retention.
- When authoring a new OpenSpec change, append exactly one unchecked final-history-retrospective task as the initially last task in `tasks.md`. Create that task only during initial change authoring; do not retrofit active or archived changes and do not let apply, archive, compaction, or the task itself create another copy. After every other currently known task is complete, execute it by applying this same six-cell `Quality` / `Cycle Speed` / `Token Economy` by `Working Repository` / `opencode-kit` analysis, admission gate, target ownership, and `Session-Derived Improvements` record contract to the complete change `history.md` instead of the current session, using the accepted change outcome as `Original User Goal`. Append every admitted improvement under `## Session-Derived Improvements` and immediately continue apply work through implementation, proof, and validation; if none passes, record `none` without inventing work. Do not rerun the final history retrospective after generated tasks.
- Task persistence does not reorder safety gates. End the reflection with one `Next-Session Action`; when the live-attempt gate is `blocked` or `unknown`, `Next Strategy` and that action SHALL name the same first gate-closing offline step, and no improvement task may preempt it. All admitted tasks still remain mandatory before normal complete archive.
...
- Outcome-first simplicity: before a new dependency, top-level mechanism/API, out-of-owner infrastructure, multi-implementation abstraction, or duplicate behavior, load `reuse-discovery` before production. Record `reuse | extend | build-minimal`, contract/total-cost reason, and `synced | pending | not-applicable`; local fixes/data/config/generated/mechanical edits and selected-API glue do not trigger. First remove unnecessary capability; narrow users/data/interfaces; reuse an existing platform/project mechanism; then a local guard/test. Multiple coordination/recovery/compatibility/policy mechanisms need narrower scope or proof.
...
- `question` labels only capture selection and must not carry required context. Never ask for a bare retry, specialist launch, plan/spec/task update, successor attempt, attempt-limit change, revision/process approval, or budget extension.
