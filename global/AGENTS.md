...
- Every compaction summary SHALL state `Original User Goal`, `Goal Status`, and `Session Reflection` with outcome, bottleneck, evidence, uncertainty, and what not to repeat. Emit `Live-Attempt Gate: clear | blocked | unknown` for its path/lane; when not `clear`, add `Failure Chain`, `Preserved Raw Bundles`, `Offline Replay Coverage`, `Terminal Replay Result`, and `Unlock Condition`. Name the path/lane; routes/outcomes never clear it. Retain mixed status as explicit dimension/value, not a claim or context pointer. Missing evidence is `unknown`.
