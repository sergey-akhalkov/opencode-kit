...
export function ownershipGateChecks(options: GateReaderOptions): OpenSpecOperationGateCheck[] {
  ...
  if ((options.operation === "apply" || options.operation === "archive") && changeId != null) {
...
export function doctorOwnershipChecks(root: string, enforcement?: OwnershipEnforcement): Array<{
  name: string;
  ...
  if (findings.length === 0) {
    return [{
      name: "openspec ownership",
      status: "pass",
      detail: "Active OpenSpec ownership inventory has no advisory findings.",
...
export function archiveOwnershipBlocker(root: string, changeId: string, enforcement?: OwnershipEnforcement): string | null {
  const checks = ownershipGateChecks({ root, operation: "archive", changeId, enforcement });
