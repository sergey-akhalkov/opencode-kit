...
- Risk-driven test discipline: implementation-capable artifacts minimize time-to-first-real-signal, require the earliest safely reachable real boundary before dependent expansion, and keep exact blocker/unblocker and authorization/safety/restoration gates when deferred. This production proof precedes systematic test design; Material/explicit qualification then uses independent fresh-context critical test authoring, while Ordinary Small uses focused validation and optional smallest post-proof regression.
