...
- When doc/spec hardening creates behavior-changing tasks, require original requirements, minimal happy-path implementation, and observable proof first. Main may add the smallest focused regression after proof; independent fresh test-only negative/end-to-end authoring is required only for a reachable named critical consequence or explicit project/owner requirement.
