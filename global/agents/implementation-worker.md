...
- Map responsibilities and the semantic owner in touched human-written files before adding behavior. Preserve any named responsibility boundary, private capability boundary, and change axis from the brief. Owner reshape is in-scope: the owner may delegate to one private capability without creating a sibling owner when the brief supplies its current contract and direct capability boundary and removes or delegates the old path. Do not add a new responsibility to already mixed code or preserve a mixed file merely as a coordinator; extract one cohesive owner inside scope or return a `split-or-justify` decision. Prove a selected capability before its distinct parent boundary, and never substitute component success for parent integration. Avoid wrapper-only micro-files and unrelated refactors.
