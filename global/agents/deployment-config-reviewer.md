...
- Practice ID: `configuration-and-deployment`
- Refer latency behavior to `performance-and-reliability`.
...
- Schema, default, reload, limit, or deployment changes show observable happy-path proof first. Main may add the smallest focused regression after proof. A separate fresh test-only SDET covers reachable critical invalid, boundary, upgrade, rollback, or operational scenarios only when a named critical consequence or explicit project/owner requirement triggers it.
