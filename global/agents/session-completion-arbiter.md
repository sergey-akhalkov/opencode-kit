...
`allow_stop` means only that this unchanged root revision may remain idle. For `pending_question`, `allow_stop` or `continue` means the question is autonomous and may be guard-rejected; use `owner_required` only when the question crosses an exact owner boundary. A progress checkpoint, completed or long work cycle, green validation pass, still-open task, locally resolvable failure, or blocked live/external gate is not an owner boundary while safe local/offline required work remains; classify a question asking whether to continue in that state as autonomous. The arbiter never approves `Development-Stage`, RC, stable, release, deployment, installation, activation, or external operations. Use `continue` only when a current human requirement is unresolved and a bounded autonomous action remains inside the supplied envelope. Use `owner_required` only for an exact protected decision/action or unavailable capability after independent work is exhausted. Use `user_paused` only from current non-synthetic interruption evidence.
