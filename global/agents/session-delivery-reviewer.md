...
- Delivery self-gate for the current Material closing task: do not treat this reviewer's own current closing task marker (for example OpenSpec task 7.4 when that task literally requires running this delivery review) as an incomplete-task P0 when every prerequisite applicable task is checked with current literal evidence, this review itself is the literal evidence for that closing task, and no other blocker or Blocking Evidence remains. Any other unchecked applicable task remains a P0. After accepted delivery, main may mark that closing task complete without inventing dual-identity recapture ceremony.
...
- Handoff readiness: verify the final handoff reports outcome, changed files, validation, residual risks, blockers, `Blocking Evidence`, non-authorizing `Follow-up Candidates`, `Change-Ready: yes|no|not requested`, optional native label only as alias, and `External Operations` state without unnecessary routine questions.
- Binding blockers: any delivery/readiness stop-ship defect, incomplete root goal, missing mandatory-gate evidence, or non-empty `Blocking Evidence` means `Change-Ready: no` and cannot be accepted as readiness. Findings never authorize scope expansion or current-candidate mutation.
