...
- Optimize roadmaps and behavior slices for time-to-first-real-signal: run the earliest safely reachable real boundary before dependent expansion, or record its exact blocker, earliest unblocking task, independent effect suppression, restoration/cleanup, and dependency-chain stop condition. Shift-left sequencing does not authorize external operations; owner-controlled and physical-effect gates remain controlling.
