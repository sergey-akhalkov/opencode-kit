...
- Make `stopManagedServer()` succeed when the validated supervisor, OpenCode server-root/listener, and Graphify root/listener identities are gone, even if unmatched descendants already exited or refuse `taskkill /T`.
...
### Decision 1: Kill only the validated managed identities

`stopManagedServer()` already resolves the supervisor, OpenCode server-root/listener, and Graphify root/listener through `validateManagedStopState()`. After that check, terminate those PIDs individually with `taskkill /PID <id> /F` and without `/T`. Order: server-root, OpenCode listener, Graphify root, Graphify listener, supervisor. Killing the OpenCode listener first lets `opencode serve` respawn an orphan on `4096` (live stop 2026-08-19: recorded PIDs gone, one leftover listener, task `Ready`). After the recorded kills, terminate only leftover listeners whose parentage matches the validated OpenCode or Graphify root/supervisor identity. Then `stopServerTask()`. Wait until those PIDs are gone and both managed-port listener sets are empty. `restart()` keeps the existing “no leftover listener before `startServerTask()`” gate for both services.
...
Reuse: extend the existing dual-service stop owner. Do not add a new killer or package.
...
- **[Risk] A non-validated OpenCode or Graphify descendant keeps running after stop** → Accepted unless it still owns `4096` or `4097`; a leftover listener remains fail-closed and blocks replacement.
- **[Risk] Killing a listener PID without `/T` leaves its root running long enough to rebind** → Kill the OpenCode server-root before its listener, then the Graphify root/listener and supervisor; wait for PID and both listener sets to become absent before start; fail if either port is re-owned by an unmatched process.
...
3. Re-verify elevation, stop the managed runtime if the installer requires it, repair the complete protected ProgramData package and tray, confirm manifest hashes.
4. One live tray Restart: new managed OpenCode and Graphify identities, green dual-service health, no new `restart` error line.
5. Prove fail-closed unmatched listener and Desktop Restart through `invoke.vbs`.
6. Fresh critical-only SDET, then `node --check`, installed help/preflight/status, `npm test`, `npm run validate:strict`, and `npx openspec validate fix-workstation-restart-reliability --strict`.
