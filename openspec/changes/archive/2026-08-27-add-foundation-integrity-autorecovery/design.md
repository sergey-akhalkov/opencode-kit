...
runtime profiles, validators, catalogs, and configured-session proof.
`replace-instruction-limits-with-context-quality` must apply first, or this change
must explicitly rebase before mutation. New discovery metadata and bodies require
canonical ownership, exact-duplicate detection, mechanical canonicalization,
context-quality checks, and loaded behavior proof. Inventory size and token-proxy
measurements remain diagnostics rather than acceptance ceilings.
...
- Reuse current profile, validation, proof-client, evaluator, cleanup, and claim-
  evidence mechanisms.
- Preserve context quality and loaded behavior for added runtime text without
  weakening existing safety or owner triggers or deleting unique contracts merely
  to reduce diagnostic measurements.
...
### 7. Qualify instruction growth without a size proxy

New descriptions remain trigger-complete and concise. Added owner/skill bodies rely
on inherited global reviewer, Practice Owner, authority, worktree, and evidence
contracts instead of copying them.

Implementation removes verified duplicate boilerplate only where the same operative
contract remains available through one canonical owner. Owner-specific triggers,
exclusions, inputs, outputs, permissions, and failure behavior remain local.
Description curation preserves the existing keyword and negative routing tests for
every touched artifact.

Inventory before/after remains diagnostic. Exact profile catalogs, canonicalization,
exact-duplicate detection, context-quality checks, strict validation, and matched
behavior packs bound the change. Another required core owner or skill is not removed
merely to reduce a size or token-proxy measurement.

Alternative rejected: restoring numeric instruction maxima would make a size proxy
an acceptance gate even though within-limit text can still duplicate authority or
regress loaded behavior.
...
- **Context cleanup could weaken existing behavior** -> Remove only evidenced exact or
  inherited duplicate clauses, preserve specialized contracts and descriptions, and
  replay affected structural and configured behavior tests before retention.
...
3. Apply the instruction-context replacement contract, then retain before/after
   diagnostics, canonicalization, context-quality, and regression results; consolidate
   only evidenced duplication with one canonical owner.
