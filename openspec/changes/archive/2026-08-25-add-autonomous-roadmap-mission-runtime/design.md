...
The Product Candidate is the content-bound launcher, executor, controller/preflight/contracts, installed config/validator changes, and runtime docs. Proof-attempt labels do not define Product Candidate identity. The Proof Runner drives the installed OpenCode and slash-command boundary in disposable projects and captures raw facts. The Evaluator reads preserved bundles and derives scenario verdicts without driving providers or mutation. The configured Environment Identity includes OpenCode, kit source digests, pinned PTY package, OpenSpec CLI, Node/Bun runtime, selected configured model/profile/variant, and disposable project digest. Provider-free and interruption lanes preserve their own exact Environment Identities and claim ceilings; they are not represented as one runtime execution merely because current source hashes overlap.

Task 6.1 may compose current generated-`all` install/doctor/loader evidence with an actual loaded isolated slash-command capture when both sides bind the same product source hashes. This composition proves materialization plus loaded behavior, not the abandoned generated-`all` r1-r6 provider-resolution mechanism. Its Candidate Reference must name every composing bundle, runner/evaluator identity, environment boundary, and known limitation.
...
4. one bounded configured-provider generic mission covering propose, apply, archive, local commit, and auto-chain, with separately attributed current provider-free or local lanes for question, compaction, and local-blocker recovery;
...
For the retained OpenCode 1.18.22 qualification environment, proof-server readiness is bounded to 180 seconds: isolated config must load within 15 seconds and configured/internal plugins receive the remaining 165 seconds. This is a proof-environment startup bound only; session executor, model, command, stop, cleanup, and mutation timeouts are unchanged.
