...
The managed compaction prompt in `global/opencode.json.template` carries only unresolved execution facts: current leaf, blocked parent, dependency and gate refs, next oracle, and evidence refs. It does not preserve an entire conceptual tree, repeat decomposition analysis, or create a second compaction block type.
...
`add-outcome-preserving-delivery-checkpoints` archived as `2026-09-01-add-outcome-preserving-delivery-checkpoints` after terminal writer closure. Current canonical specs, loaded main, the managed compaction prompt, OpenSpec apply, completion arbiter/frontier proofs, and the consumer-outcome proof family were reread; the exact 19 overlapping roots are now transferred to this sole mutation-enabled owner.

Non-overlapping planning surfaces such as propose and `deep-task-planning` were not implemented early because the installed behavior claim requires one coherent candidate across all owners. The ownership manifest preserves the terminal predecessor dependency and acquired transfer while successor changes remain mutation-disabled.
...
- Configured proof: inspect candidate/environment/model identities, exact fixtures, calls, events, file effects, current-run diagnostics, and cleanup before temporary output is removed.
...
1. Confirm archived `add-outcome-preserving-delivery-checkpoints`, terminal writer closure, and current canonical specs, source, proof owners, and runtime identities.
2. Preserve the recorded exact 19-root transfer and this change as the sole mutation-enabled candidate.
