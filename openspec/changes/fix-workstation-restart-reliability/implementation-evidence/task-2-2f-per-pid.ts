...
const evidenceName = "task-2-2f-raw.json"
...
function killServeWscript() {
  const listing = spawnSync("powershell.exe", [
    ...
    "-NoProfile",
    "-Command",
    "$ids = @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -and $_.CommandLine -like '*invoke.vbs*serve*' } | ForEach-Object { [int]$_.ProcessId }); $ids -join ','",
...
const record: Record<string, unknown> = {
  ...
  delayMs,
  mechanism: "per-pid-taskkill-then-install-start-tray-remote-graphify",
...
try {
  ...
  if (!portGone.ok) throw new Error("port 4096 still listening after per-pid stop")
  ...
  const healthy = waitUntil("healthy", 90_000, () => Boolean(parseStatus(run([controller, "status"], 30_000).stdout)?.health?.healthy))
