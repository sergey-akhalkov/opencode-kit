...
Use this exact concise discovery contract: `Use by default after initial foraging for a new non-trivial parentless root mission; returns the smallest sufficient main/skill/subagent team. Stay quiet for one existing-owner local action with known proof.` The body owns the complete objective bypass and output semantics; the description remains concise, canonically owned, context-qualified, and covered by loaded discovery proof. Inventory size remains diagnostic.
...
The tool uses the official current OpenCode runtime agent/skill listing surface and root-session identity when available. Implementation preflight must verify the exact current API from installed source or published schema before behavior or profile mutation. The generated runtime-surface manifest is used only as a repository-managed profile/source cross-check, not as a substitute for project-level or merged runtime availability. If the official runtime cannot expose a complete root-effective catalog, implementation stops this lane and revises the smallest catalog adapter with explicit `unknown`; it does not fall back to copied prompt rosters or semantic path inference.
...
This change remains planning-only until shared write ownership is conflict-free. Materialize `ownership.json` before freezing `tasks.md`; keep mutation disabled and give initial tasks only create-new advisor/catalog/proof-fixture roots until every shared path is terminal or explicitly transferred. Reconcile these active changes:

- `add-bounded-falsification-review`: owns readiness-review routing and overlapping `global/AGENTS.md`, profiles, agent contracts, and proof surfaces;
- `add-autonomous-campaign-orchestration`: owns campaign-level dynamic wave planning but not single-root team advice;
- `add-continuous-complexity-management`: may own overlapping main-context and skill-routing text;
- `add-foundation-integrity-autorecovery`: may own Practice Owner registry/profile/catalog validation roots.

Implementation starts only after each overlapping writer is terminal or explicitly transfers exact paths. Semantic reconciliation preserves bounded falsification as an advisor-recommendable challenge, campaign orchestration as a separate multi-session control plane, complexity management as a recommendable skill, and foundation recovery as structural integrity rather than team selection.
...
- **Current Rung:** Rung 0, reviewed planning artifacts with no runtime candidate or observations.
