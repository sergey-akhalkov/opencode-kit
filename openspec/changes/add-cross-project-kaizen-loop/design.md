...
The managed compaction prompt always emits one fenced, versioned Kaizen JSON envelope after the existing required reflection fields. `signals: []` is the only valid no-finding representation. The envelope is capped at three entries and uses the signal schema subset.
...
Once proven, the hook reads only the correlated summary, extracts exactly one fenced envelope, discards all other text, validates/redacts, appends idempotently, and completes within a short local deadline. Invalid/missing output creates a bounded gap event and leaves compaction continuation unchanged.
