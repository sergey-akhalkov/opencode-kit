...
The current rung is reviewed planning against current canonical instructions, specs, archived change-locality/reuse decisions, and proof inventories. After exact ownership transfer, the next real boundary is provider-free materialization and red-control replay for the reviewed `CCO-001` scenario population, followed by matched installed authoring and reuse decisions in disposable repositories.

Later configured captures use the existing bounded synthetic-provider authority, isolated disposable roots, exact source/model/profile/permission identity, no consumer-project mutation, no public network or dependency installation for fixture decisions, and terminal process/session/root cleanup. Evidence retains exact invocation, exit status, stdout/stderr, relevant events and exceptions, changed-file manifests, direct and parent observations, evaluator facts, and artifact paths. A failed configured capture must reach terminal provider-free replay before an equivalent capture; process attempt controls remain revisable and do not authorize provider, install, activation, publication, or remote effects.
...
Extend `tools/proofs/agent-tooling-ergonomics.ts` with a reviewed capability-composition scenario pack or the smallest equivalent extension of its current change-locality data owner. It already supports disposable authoring, source/test/runtime observations, changed-file manifests, configured captures, provider-free replay/evaluation, and cleanup. The new pack covers private extraction, cohesive direct behavior, wrapper/duplicate controls, delegation, leaf/module independence, and component-versus-parent proof.
...
`add-leaf-first-task-decomposition` currently plans mutations to loaded main, Change-Ready, production planning, exact marker contracts, and installed proof owners. This change remains planning-only and mutation-disabled until that change archives, every mutation-capable writer is terminal, current canonical source/spec/proof identities are reread, and explicit ownership transfer yields one conflict-free candidate.
