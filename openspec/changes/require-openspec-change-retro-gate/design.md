...
- Durable output as OpenSpec follow-up changes in the current project or reusable OpenSpec proposals/changes for `https://github.com/uplift-labs/opencode-dev-kit`.
- No archive until the retro result is recorded or an approved skip reason exists.
- Future automation so this is enforceable rather than only a reminder.
...
| Problem | Evidence | Impact | Recommendation | Target |
| --- | --- | --- | --- | --- |
...
- Concrete findings exist but no project follow-up change, `opencode-dev-kit` proposal, or justified no-action decision is recorded.
...
## Skill And Workflow Updates To Implement Later

The implementation change should update these artifacts:

| Artifact | Required Update |
| --- | --- |
| `openspec-archive-change` skill | Refuse or block archive if the retrospective gate is missing, incomplete, or skipped without approval. |
...
A future TypeScript helper can make the rule enforceable:
...
- Findings either route to project-local OpenSpec changes, `opencode-dev-kit` proposals/changes, or explicit no-action decisions.
...
2. Create or update an OpenSpec proposal/change in the current `opencode-dev-kit` repository when already working there.
...
- A prose-only rule may be forgotten. Mitigation: add a deterministic helper and skill/archive enforcement in implementation.
