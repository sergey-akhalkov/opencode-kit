...
4. `Small Slice`: implement the smallest useful increment as a vertical slice that minimizes time-to-first-real-signal; for compound work plan parent-to-leaf and execute/prove dependency-valid evidence-bearing leaves before the parent's distinct integration oracle, while cohesive Ordinary Small and same-oracle mechanical work stay direct. Prefer remove/narrow/reuse/local guard, preserve exact gates and unrelated work, and use `split-or-justify` rather than adding a responsibility to an already mixed file.
5. `Happy Path`: main is the default production author for Ordinary Small and Material.
6. `Runtime Proof`: run-observe-correct at the first safely reachable real boundary sufficient to observe the accepted effect; inspect invocation/input, identity, exit status, stdout/stderr, relevant logs/exceptions, side effects, artifact paths, and cleanup from the current run. Climb from integration/simulator to bounded live or end-to-end only for a current requirement, invariant, or unresolved equivalence risk. Compilation or unit checks alone are not proof.
