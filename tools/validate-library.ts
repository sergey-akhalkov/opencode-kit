...
for (const required of REUSABLE_REVIEWER_LEAF_CONTRACT_TEXT) {
    ...
    if (REUSABLE_REVIEWER_FORBIDDEN_INLINE_BLOCKS.some((pattern) => pattern.test(text))) {
      addError(`Reusable reviewer agent must reference the shared contract via ## Contract Reference, not inline the Leaf Contract or Prevention Feedback body: ${file}`);
    }
