...
Record project-specific commands here. Keep the Universal Development Loop and shared lifecycle runtime unchanged; this file only adapts validation commands to the current technology stack.
...
- Autonomous campaign readiness separately requires resolved `opencode-dev-kit/work-campaign.json` and `opencode-dev-kit/work-campaign-adapter.json` files. Bootstrap and doctor inspect these files but never enable provider use or install, register, start, or resume a supervisor.
